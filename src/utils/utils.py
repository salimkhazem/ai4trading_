...
def load_day_data(base_path: str, day: str) -> pd.DataFrame:
    ...
    if not parquet_files:
        raise FileNotFoundError(f"No parquet files found for {day} in {day_path}")

    start_time = time.time()
    try:
        dfs = [pd.read_parquet(f) for f in parquet_files]
        if not dfs:
            raise ValueError(f"Failed to read any parquet files for {day}")
        df_day = pd.concat(dfs, ignore_index=True)
    except Exception as e:
        raise ValueError(f"Error reading or concatenating parquet files for {day}: {e}")
    ...
    if 'time' in df_day.columns:
        ...
        raise ValueError(f"'time' column not found for {day}")

    # Add trading day as separate column
    try:
        df_day['date'] = pd.to_datetime(day, format='%Y%m%d')
    except ValueError:
        raise ValueError(f"Invalid date format for folder: {day}")
