...
class HFTDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.intermediate_dir = self.base_data_dir # Daily data is now directly under base_data_dir/day

        self.X_windows = None
        self.target_windows = None
        self.window_info = None
        self.labels = None # Store calculated labels
        self.relative_change = None # Store relative change for analysis
        self.lower_tercile_threshold = None # Initialize tercile thresholds
        self.upper_tercile_threshold = None # Initialize tercile thresholds
    ...
    def _calculate_relative_change(self) -> np.ndarray:
        ...
        """
        # --- BEGIN concise alignment pre-check --- (This can remain as a general sanity check)
        wmp_mean_feature_idx = 0 # Based on features.txt
        expected_info_col = None
        if self.bar_type == 'volume':
            expected_info_col = 'last_target_in_window'
        elif self.bar_type == 'time':
            expected_info_col = 'last_wmp_in_window'

        # if expected_info_col and expected_info_col in self.window_info.columns:
        #     try:
        #         x_check_vals_pre = self.X_windows[:, -1, wmp_mean_feature_idx]
        #         info_check_vals_pre = self.window_info[expected_info_col].values.astype(x_check_vals_pre.dtype, copy=False)
        #         if not np.all(np.isclose(x_check_vals_pre, info_check_vals_pre)):
        #             num_mismatches_pre_check = np.count_nonzero(~np.isclose(x_check_vals_pre, info_check_vals_pre))
        #             logging.error(
        #                 f"CRITICAL ALIGNMENT ERROR in _calculate_relative_change precondition: "
        #                 f"{num_mismatches_pre_check} mismatches detected. "
        #                 f"X_windows[:, -1, {wmp_mean_feature_idx}] vs window_info['{expected_info_col}']."
        #             )
        #             mismatch_indices_pre = np.where(~np.isclose(x_check_vals_pre, info_check_vals_pre))[0]
        #             for i in mismatch_indices_pre[:3]:
        #                 logging.error(f"  Mismatch Example - Index {i}: X_val={x_check_vals_pre[i]:.8f}, Info_val={info_check_vals_pre[i]:.8f}, Diff={(x_check_vals_pre[i] - info_check_vals_pre[i]):.8e}")
        #             raise AssertionError(
        #                 "Data misalignment detected in _calculate_relative_change precondition. Check logs."
        #             )
        #     except Exception as e:
        #         logging.error(f"Error during concise alignment pre-check in _calculate_relative_change: {e}", exc_info=True)
        #         raise AssertionError("Error during alignment pre-check. Check logs.")
        # elif expected_info_col:
        #     logging.warning(
        #         f"Alignment pre-check in _calculate_relative_change skipped: "
        #         f"Column '{expected_info_col}' not found in window_info. Columns available: {self.window_info.columns.tolist()}"
        #     )
        # # --- END concise alignment pre-check ---

        logging.info("Calculating relative change using X_windows directly for initial value...")
        if self.X_windows is None or self.target_windows is None:
            raise ValueError("X_windows or target_windows must be loaded before calculating relative change.")

        # Use X_windows directly for the last value of the input window.
        # Ensure wmp_mean_feature_idx (e.g., 0) corresponds to WMP in your features.txt
        last_val_in_input_window = self.X_windows[:, -1, 0].astype(np.float64)
